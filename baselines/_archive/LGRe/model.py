...
class LGRe(torch.nn.Module):
    ...
    def forward(self, sub, rel, year, month, day, neg_ents, strategy='one_to_x'):
        ...
        if isinstance(loss_t, tuple):
            loss_t = torch.cat([loss_t[0], loss_t[1], loss_t[2]], dim=0)
        return pred, loss_t
    ...
    def temporal_emb(self, y_emb, m_emb, d_emb):
        if self.p.dataset == 'icews14':
            ...
            final_tensor = torch.vstack(combined_list)
        elif self.p.dataset == 'xcad':
            # xcad: 2020-07-01 to 2020-08-30 (61 days)
            combined_list = []
            init_year_data = y_emb.weight  # [1, dim]
            init_month_data = m_emb.weight  # [2, dim]
            init_day_data = d_emb.weight    # [31, dim]
            # July: days 1-31, August: days 1-30
            year_data = init_year_data[0].unsqueeze(0).repeat(61, 1)  # [61, dim]
            month_data_july = init_month_data[0].unsqueeze(0).repeat(31, 1)  # [31, dim]
            month_data_aug = init_month_data[1].unsqueeze(0).repeat(30, 1)   # [30, dim]
            month_data = torch.cat([month_data_july, month_data_aug], dim=0)  # [61, dim]
            day_data = torch.cat([init_day_data[:31], init_day_data[:30]], dim=0)  # [61, dim]
            year_x = self.act_t(self.simple_t(torch.cat([year_data, month_data, day_data], dim=1)))
            month_x = month_data
            day_x = day_data
