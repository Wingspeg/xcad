...
def continual_test(model, history_list, data_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, mode):
    ...
    for time_idx, test_snap in enumerate(data_list):
        ...
        _, rank_raw, rank_filter, final_r_score = model.predict(valid_history_glist, valid_history_super_glist, num_rels, valid_tensor, use_cuda,
        ...
        _, rank_raw, rank_filter, final_r_score = model_initial.predict(test_history_glist, test_history_super_glist, num_rels, test_tensor, use_cuda,
        ...
        _, rank_raw, rank_filter, final_r_score = model.predict(test_history_glist, test_history_super_glist, num_rels, test_tensor, use_cuda,
                                                                 all_ans_ent=all_ans_list[tc], eval_bs=1000)
        mrr_filter_test_snap_r, mrr_test_snap_r, rank_raw_r, rank_filter_r = utils.get_total_rank(test_tensor, final_r_score, all_ans_r_list[tc], eval_bz=1000, rel_predict=1)
        mrr_filter_test_snap = torch.mean(1.0 / rank_filter.float()).item()
        mrr_test_snap = torch.mean(1.0 / rank_raw.float()).item()
...
def test(model, history_len, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, mode):
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)): # 对于每一个待测试的时间戳 snapshot_list，按时间戳划分的事实array：[[[s, r, o], [], ...], [], ...]
        ...
        test_triples, rank_raw, rank_filter, final_r_score = model.predict(
            history_glist, history_super_glist, num_rels, test_triples_input, use_cuda,
