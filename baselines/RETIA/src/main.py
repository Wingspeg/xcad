...
from rgcn.utils import build_sub_graph, get_relhead_reltal, build_super_g, sort_and_rank
...
def continual_test(model, history_list, data_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, mode, static_graph):
    ...
    for time_idx, test_snap in enumerate(data_list):
        ...
        _, rank_raw, rank_filter, final_r_score = model.predict(valid_history_glist, valid_history_super_glist, num_rels, static_graph, valid_tensor, use_cuda,
        ...
        _, rank_raw, rank_filter, final_r_score = model_initial.predict(test_history_glist, test_history_super_glist, num_rels, static_graph, test_tensor, use_cuda,
        ...
        _, rank_raw, rank_filter, final_r_score = model.predict(test_history_glist, test_history_super_glist, num_rels, static_graph, test_tensor, use_cuda,
                                                                 all_ans_ent=all_ans_list[tc], eval_bs=1000)
        mrr_filter_test_snap_r, mrr_test_snap_r, rank_raw_r, rank_filter_r = utils.get_total_rank(test_tensor, final_r_score, all_ans_r_list[tc], eval_bz=1000, rel_predict=1)
        mrr_filter_test_snap = torch.mean(1.0 / rank_filter.float()).item()
        mrr_test_snap = torch.mean(1.0 / rank_raw.float()).item()
...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, mode, static_graph):
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)): # 对于每一个待测试的时间戳 snapshot_list，按时间戳划分的事实array：[[[s, r, o], [], ...], [], ...]
        ...
        # predict 签名改为含 all_ans_ent；返回 rank_raw/rank_filter 不再调 get_total_rank
        test_triples, rank_raw, rank_filter, final_r_score = model.predict(
            history_glist, history_super_glist, num_rels, static_graph, test_triples_input, use_cuda,
            all_ans_ent=all_ans_list[time_idx], eval_bs=1000)

        # ----- 按 relation 分组收集 rank -----
        rel_ids = test_triples[:, 1].cpu()         # [2*N_snap], relation id 含逆 (predict 内部 cat 了 inverse)
    ...
    if args.test_valid and os.path.exists(test_state_file):
        ...
        for epoch in range(args.n_epochs): # 对于每一个epoch
            ...
            for train_sample_num in tqdm(idx): # 对于每一个时间戳（idx编号是打乱的，但是还对应相应的第几个时间戳）
                ...
                # 边采样：限制传给 get_loss 的目标 triples 规模
                if output[0].shape[0] > MAX_TRIPLES_PER_STEP:
                    indices = torch.randperm(output[0].shape[0])[:MAX_TRIPLES_PER_STEP]
                    output[0] = output[0][indices]
