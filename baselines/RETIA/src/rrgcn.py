...
class RecurrentRGCN(nn.Module):
    ...
    def predict(self, test_graph, test_super_graph, num_rels, static_graph, test_triplets, use_cuda,
                all_ans_ent=None, eval_bs=1000):
        """
        分批算 entity score + 分批算 rank，relation 路全量计算。
        峰值显存 ≈ [eval_bs, num_ents] ≈ 500MB (bs=1000, num_ents=124891, FP32)。

        返回: (all_triples, rank_raw, rank_filter, scores_rel)
          - rank_*: 1-indexed rank 张量, shape=[2*N]
          - scores_rel: [2*N, num_rels*2]，relation 路维度极小不 OOM，全量返回
          - entity 与原 utils.get_total_rank 返回的 rank 完全等价
        """
        with torch.no_grad():
            ...
            # evolve_embeddings / rel_embeddings 只在分块循环外算一次
            evolve_embeddings = []
            rel_embeddings = []
            ...
            for batch_start in range(0, N, eval_bs):
                ...
                triples_batch = all_triples[batch_start:batch_end]

                score_list = self.decoder_ob.forward(evolve_embeddings, rel_embeddings, triples_batch, mode="test")
                ...
                # 2) filter rank: all_ans_ent[(h, r)] 只与本 batch 的 triples 有关，不跨 batch
            ...
            # === relation 路：保留原样（全量，维度极小不爆）===
            score_rel_list = self.rdecoder.forward(evolve_embeddings, rel_embeddings, all_triples, mode="test")
            ...
            return all_triples, rank_raw, rank_filter, scores_rel
