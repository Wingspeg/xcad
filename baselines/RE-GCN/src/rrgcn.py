...
class RecurrentRGCN(nn.Module):
    ...
    def predict(self, test_graph, num_rels, static_graph, test_triplets, use_cuda,
        ...
        """
        分批算 score + 分批算 rank, 避免 [2*N_test, num_ents=124891] 整矩阵 OOM。
        峰值显存 ≈ [eval_bs, num_ents] ≈ 500MB (bs=1000, num_ents=124891, FP32)。

        返回: (all_triples, rank_raw, rank_filter, rank_raw_r, rank_filter_r)
          - rank_*: 1-indexed rank 张量, shape=[2*N_test]
        ...
        with torch.no_grad():
            ...
            for start in range(0, N, eval_bs):
                ...
                batch_triples = all_triples[start:end]   # [bs, 3], 语义与原 get_total_rank 切片一致

                # === entity score: [bs, num_ents] ===
                score = self.decoder_ob.forward(embedding, r_emb, batch_triples, mode="test")
                target_e = batch_triples[:, 2]            # 目标列 = 尾实体 id

                # 1) raw rank (基于未过滤的 score)
                rank_raw_list.append(sort_and_rank(score, target_e))

                # 2) filter rank: 原地把已知正确答案的 score 设为 -1e7 (与 utils.filter_score 行为一致),
                #    然后再算一次 rank。all_ans_ent[(head, rel)] 的 key 只与本 batch 的 triples 有关,
                #    不会跨 batch 漏过滤或错位。
                ...
                # === relation score: [bs, 2*num_rels], 维度小, 不 OOM ===
                score_rel = self.rdecoder.forward(embedding, r_emb, batch_triples, mode="test")
                target_r = batch_triples[:, 1]            # 目标列 = 关系 id
    ...
    def get_loss(self, glist, triples, static_graph, use_cuda):
        ...
        if self.entity_prediction:
            # 大实体空间(124891)下一次性算 [N, num_ents] 会 OOM, 改为分批 sum-CE 再除 N 取平均
            ent_eval_bs = 2000
            ...
            for start in range(0, num_rows, ent_eval_bs):
                ...
                loss_ent_sum = loss_ent_sum + F.cross_entropy(
                    batch_scores, batch_triples[:, 2], reduction="sum"
                )
                del batch_scores
