...
class UnionRGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, num_bases=-1, bias=None,
                 ...
                 use_rel_decay=False, rel_decay=None):
        ...
        self.activation = activation
        self.self_loop = self_loop
        self.num_rels = num_rels           # 已含正逆: 8 (num_rels*2 from caller)
        self.rel_emb = None
        self.skip_connect = skip_connect
        ...
        # 所有关系共享的 weight_neighbor (use_hetero=False 时使用)
        ...
        # 异质 basis 分解 (use_hetero=True 时创建)
        if self.use_hetero:
            self.msg_basis = nn.Parameter(torch.Tensor(n_msg_basis, in_feat, out_feat))
            nn.init.xavier_uniform_(self.msg_basis, gain=nn.init.calculate_gain('relu'))
            self.rel_msg_coeff = nn.Parameter(torch.Tensor(self.num_rels, n_msg_basis))
            nn.init.xavier_uniform_(self.rel_msg_coeff)
    ...
    def forward(self, g, prev_h, emb_rel, step=None):
        self.rel_emb = emb_rel
        self.current_step = step

        # 异质: 在 forward 开头算一次 rel_W,msg_func 直接用
        if self.use_hetero:
            # [n_basis, in_feat, out_feat] × [num_rels, n_basis]^T = [num_rels, in_feat, out_feat]
            self.rel_W = torch.einsum('bio,rb->rio', self.msg_basis, self.rel_msg_coeff)
    ...
    def msg_func(self, edges):
        ...
        edge_type = edges.data['type']   # [E], 0-7
        node = edges.src['h'].view(-1, self.out_feat)
        msg = node + relation            # [E, out_feat]
        if self.use_hetero:
            out = torch.zeros_like(msg)
            for r in range(self.num_rels):   # num_rels=8 含逆
                mask = (edge_type == r)
                if mask.any():
                    out[mask] = torch.mm(msg[mask], self.rel_W[r])
            msg = out
        else:
            msg = torch.mm(msg, self.weight_neighbor)
        # ── 关系感知时间衰减 ──
        if self.use_rel_decay and self.current_step is not None:
            edge_type = edges.data['type']   # [E], 0-7
            decay_rate = F.softplus(self.rel_decay[edge_type])  # [E]
            decay = torch.exp(-decay_rate * self.current_step)   # [E]
            msg = msg * decay.unsqueeze(-1)
        return {'msg': msg}
